...
namespace Document.Application.Features.Documents.Commands.AddEdit;
...
internal class AddEditDocumentCommandHandler : IRequestHandler<AddEditDocumentCommand, Result<int>>
{
    ...
    public AddEditDocumentCommandHandler(IUnitOfWork<int> unitOfWork, IMapper mapper, IUploadService uploadService)
    {
    ...
    public async Task<Result<int>> Handle(AddEditDocumentCommand command, CancellationToken cancellationToken)
    {
        ...
        if (command.Id == 0)
        {
            var doc = _mapper.Map<Domain.Entities.Document>(command);
            if (uploadRequest != null)
            {
                doc.URL = _uploadService.UploadAsync(uploadRequest);
            }
            await _unitOfWork.Repository<Domain.Entities.Document>().AddAsync(doc);
            await _unitOfWork.Commit(cancellationToken);
            return await Result<int>.SuccessAsync(doc.Id, "Document Saved");
        }
        else
        {
            var doc = await _unitOfWork.Repository<Domain.Entities.Document>().GetByIdAsync(command.Id);
            if (doc != null)
            {
                ...
                await _unitOfWork.Repository<Domain.Entities.Document>().UpdateAsync(doc);
                await _unitOfWork.Commit(cancellationToken);
